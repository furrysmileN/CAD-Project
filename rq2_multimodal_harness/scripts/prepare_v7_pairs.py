# -*- coding: utf-8 -*-
"""Build V7 shape-transfer pairs. No live API. Does not write V6/V6b live dirs."""
...
from rq2_harness.common import PROJECT_ROOT, atomic_write_json, sha256_file
...
def _export_variant(spec: dict[str, Any], output_root: Path, harness_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "n_ops": len(spec.get("operations") or []),
    }
...
def prepare_pairs(n: int, output_root: Path) -> dict[str, Any]:
    ...
    for pair in pairs:
        ...
        try:
            ...
            if pair["family"] in FORBIDDEN_FAMILIES:
                raise RuntimeError("旧模板 family")
            if not operations_differ_only_in_critical(pair):
                raise RuntimeError("A/B 不只差关键事实")
            if len(pair["spec_a"]["operations"]) < MIN_OPS:
                raise RuntimeError("操作数不足")
            ...
            row = {
                ...
                "family": pair["family"],
                "host": pair["host"],
                "edit_layer": KIND_LAYER.get(pair["kind"]),
                "split": "v7_probe",
                "variant": "A",
            ...
            print(
                json.dumps(
                    {"pair_id": pair_id, "host": pair["host"], "kind": pair["kind"], "eligible": eligible, "meas_a": meas_a, "meas_b": meas_b},
                    ensure_ascii=False,
                )
            )
        except Exception as exc:
            audits.append({"pair_id": pair_id, "kind": pair.get("kind"), "host": pair.get("host"), "error": f"{type(exc).__name__}: {exc}", "eligible": False})
    ...
    for row in rows:
        ...
        layer = row.get("edit_layer") or KIND_LAYER.get(row["kind"])
        by_layer[str(layer)] = by_layer.get(str(layer), 0) + 1
    old_shells = [row["family"] for row in rows if row.get("family") in FORBIDDEN_FAMILIES]
    pass_gate = bool(
        n_eligible >= 8
        and len(by_kind) >= 4
        and len(by_host) >= 3
        and not old_shells
        and by_layer.get("L2", 0) >= 1
        and by_layer.get("L3", 0) >= 1
    )
    summary = {
        "n_requested": n,
        "n_written": len(rows),
        ...
        "eligible_by_host": by_host,
        "eligible_by_layer": by_layer,
        "forbidden_family_hits": old_shells,
        "kinds_covered": sorted(by_kind),
        "pass_probe_gate": pass_gate,
        "manifest": str(output_root / "manifest_probe.jsonl"),
        "note": "不过门不得 live、不得扩大矩阵。",
    }
    write_manifest(output_root / "manifest_probe.jsonl", rows)
    _dump(output_root / "audits" / "v7_pairs_offline.json", {"summary": summary, "pairs": audits})
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-root",
        default=str(ROOT / "outputs" / "v6_information_complementarity" / "v7_shape_transfer"),
    )
