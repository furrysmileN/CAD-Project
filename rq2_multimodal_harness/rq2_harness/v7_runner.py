"""V7 shape-transfer runner. Writes only under v7_shape_transfer. Reuses frozen C3/C5 and C2B/TB assemblers."""
...
from .v6_conditions import audit_v6_payload, build_v6_messages, parse_condition
from .v6_manifest import attach_evidence_payloads, iter_tasks, read_manifest
from .v6_runner import SCHEMA_VERSION, _fingerprint, _payload_path, _redact_messages, _resolve_latent_path, _score, _state_path
...
def _build(row: dict[str, Any], condition: str, config: dict[str, Any]) -> tuple[Any, list[dict[str, Any]], dict[str, Any]]:
    if condition in DIAG_IDS:
        spec = parse_diag_condition(condition)
        if spec.text_fact:
            row = _with_text_fact(row, config)
        messages = build_diag_messages(row, spec)
        audit = audit_diag_payload(messages, spec, sample_id=row.get("sample_id"), family=row.get("family"))
        return spec, messages, audit
    spec = parse_condition(condition)
    messages = build_v6_messages(row, spec)
    audit = audit_v6_payload(messages, spec, sample_id=row.get("sample_id"), family=row.get("family"))
    return spec, messages, audit


def run_v7(config_path: str | Path, *, dry_run: bool = True, limit: int | None = None) -> dict[str, Any]:
    config = load_config(config_path)
    output_root = project_path(config["paths"]["output_root"])
    if "v7_shape_transfer" not in str(output_root).replace("\\", "/"):
        raise ValueError("V7 只能写入 v7_shape_transfer")
    split = str(config.get("split") or "probe")
    ...
    conditions = list(config.get("conditions") or list(V7_CONDITIONS))
    if not set(conditions) <= set(V7_CONDITIONS):
        raise ValueError(f"V7 条件只允许 {V7_CONDITIONS}: {conditions}")
    ...
    for task in iter_tasks(manifest, conditions, repeats):
        ...
        spec, messages, payload_audit = _build(row, task["condition"], config)
        ...
        print(f"[v7] {row['sample_id']} {spec.condition_id} r{repeat_id:02d}", flush=True)
        try:
            ...
            print(f"[v7] FAIL {row['sample_id']} {spec.condition_id}: {type(exc).__name__}: {exc}", flush=True)
            continue
        atomic_write_json(
            path,
            {
        ...
        print(f"[v7] -> {status} pred={feat.get('pred_value')}", flush=True)
    summary = {
