"""RQ2b 确认实验运行器：冻结臂 × 全量 100 样本 × 7 条件（T/I/P/TI/TP/IP/TIP）。

双臂（--arm 选择，arms.<arm> 定义行为）：
  A0：qwen3.8-max + v2 prompt + 无 repair + 无反馈（与 pilot_v2 管线一致，仅换模型）
  C ：qwen3.8-max + v3 prompt + repair R4 + 最多 2 轮反馈（schema + execution）

输入复用 pilot_v2 的冻结 manifest（同一样本、同一模态编码），state 格式与
encoding_runner 对齐（含 feedback.rounds），以便分析脚本统一处理。
"""
...
from .common import atomic_write_json, load_config, project_path, read_jsonl, safe_id, sha256_file, sha256_json
from .conditions import CONDITIONS, validate_conditions
...
from .prompting import build_messages, parse_plan_response, validate_plan
from .repair_v21 import REPAIR_VERSION, repair_plan_v21

STATE_SCHEMA = "rq2.confirm.task_state.v1"
RUN_SCHEMA = "rq2.confirm.run_summary.v1"
TERMINAL_STATUSES = {"completed", "parse_failed", "episode_failed"}


def _resolve_conditions(raw: Iterable[str] | None, defaults: list[str]) -> tuple[str, ...]:
    if not raw:
...
def _task_order(
    ...
    """与 pilot_v2 相同的固定 hash 排序：sha256(f"{seed}:{sample_id}:{condition}")。"""
...
def _state_path(output_dir: Path, sample_id: str, condition: str) -> Path:
    return output_dir / "state" / safe_id(sample_id) / f"{safe_id(condition)}.json"
...
def _archive_previous(output_dir: Path, previous: dict[str, Any], sample_id: str, condition: str) -> None:
    target = (
        output_dir
        / "history"
        / safe_id(sample_id)
...
def run_confirmation(
    ...
    *,
    ...
    if arm:
        config.setdefault("arm", arm)
    arms_block = config.get("arms") or {}
    arm_name = str(config.get("arm") or "custom")
    arm_block = dict(arms_block.get(arm_name) or {})
    fallback_output = (
        config.get("paths", {}).get("output_dir")
        or config.get("paths", {}).get("output_root")
    )
    output_dir = project_path(
        output_dir_override or arm_block.get("output_dir") or fallback_output
    )

    manifest_path = project_path(config["paths"]["manifest"])
    if not manifest_path.is_file():
        raise FileNotFoundError(f"缺少 manifest {manifest_path}；请先运行 pilot_v2 prepare")
    rows = list(read_jsonl(manifest_path))
    expected_n = int(config["n"])
    if len(rows) != expected_n:
        raise RuntimeError(f"manifest 应有 {expected_n} 行，实际 {len(rows)}")
    if limit is not None:
        rows = rows[:limit]

    plan_version = str(arm_block.get("plan_version") or config.get("prompt", {}).get("plan_version") or "v2")
    if plan_version not in {"v2", "v3"}:
        raise ValueError(f"plan_version 仅支持 v2 或 v3（确认实验）：{plan_version!r}")
    repair_rules = tuple((arm_block.get("repair") or {}).get("rules") or ())

    feedback = resolve_feedback_config({**config, "feedback": arm_block.get("feedback") or config.get("feedback") or {}})
    feedback["arm"] = arm_name
    ...
    selected = conditions or validate_conditions(list(config.get("conditions") or CONDITIONS))
    tasks = _task_order(rows, selected, seed)
    task_manifest = [
        {"order": index, "sample_id": row["sample_id"], "condition_id": condition}
        for index, (row, condition) in enumerate(tasks)
    ]
    atomic_write_json(
        output_dir / "task_order.json",
        {
            "schema_version": "rq2.confirm.task_order.v1",
            "seed": seed,
    ...
    for task_index, (row, condition) in enumerate(tasks):
        path = _state_path(output_dir, row["sample_id"], condition)
        ...
        if path.is_file():
            ...
            if not force and _should_skip(previous, task_fingerprint, dry_run):
                ...
                continue
            if not dry_run:
                _archive_previous(output_dir, previous, row["sample_id"], condition)
        ...
        try:
            ...
            for round_index in range(int(feedback["max_rounds"]) + 1):
                ...
                round_record: dict[str, Any] = {"round": round_index, "temperature": round_temperature}
                ...
                round_record["api"] = {key: value for key, value in api_result.items() if key != "text"}
                ...
                if not isinstance(original_plan, dict):
                    ...
                    status = "parse_failed"
                    if SCHEMA_SOURCE in feedback["sources"] and round_index < int(feedback["max_rounds"]):
                        current_messages = feedback_turn(
                            messages, raw_response, build_schema_feedback(issues, {}, round_index)
                        )
                        continue
                    break
                ...
                if repaired_issues:
                    ...
                    if SCHEMA_SOURCE in feedback["sources"] and round_index < int(feedback["max_rounds"]):
                        current_messages = feedback_turn(
                            messages,
                            raw_response,
                            build_schema_feedback(repaired_issues, repaired_plan, round_index),
                        )
                        continue
                    break
                ...
                feedback_state["rounds"].append(round_record)
                status = "episode_failed"
                if EXECUTION_SOURCE in feedback["sources"] and round_index < int(feedback["max_rounds"]):
                    current_messages = feedback_turn(
                        messages,
                        raw_response,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="运行 RQ2b 确认实验（冻结臂 × 100 样本 × 7 条件）")
    parser.add_argument(
        "--config",
        default=str(Path(__file__).resolve().parents[1] / "configs" / "confirm_n100.yaml"),
    )
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--limit", type=int)
    parser.add_argument("--conditions", nargs="*")
    parser.add_argument("--arm", choices=["A0", "C", "custom"], help="覆盖 config 的 arm")
    parser.add_argument("--output-dir", help="覆盖 arms.<arm>.output_dir")
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args(argv)
    config = load_config(args.config)
    conditions = _resolve_conditions(args.conditions, list(config.get("conditions") or CONDITIONS))
